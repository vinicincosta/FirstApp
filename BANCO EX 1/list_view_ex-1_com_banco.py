...
def main(page: Page):
        ...
        for usuario in resul_usuario:
            lv_usuario.controls.append(
                ft.ListTile(
                    width=500,
                    animate_size=500,
                    leading=ft.Icon(ft.Icons.PERSON, color=Colors.BLACK),
                    title=ft.Text(f'Nome - {usuario.nome}', color=Colors.BLACK),
                    subtitle=ft.Text(f'Informações', color=Colors.BLACK),
    ...
    def salvar_usuarios(e):
        try:
            global obj_user
            if (input_nome.value == '' or slider_salario.value == ''
